...
def text_normalization(text):
    ...
    tk1=nltk.word_tokenize(spl_char_text)
    lematz=wordnet.WordNetLemmatizer()
    tags_list=pos_tag(tk1,tagset=None)
    lematz_words=[]
    for token,pos_token in tags_list:
        if pos_token.startswith('V'):
            ...
            pos_val='n'
        lematz_token=lematz.lemmatize(token,pos_val)
        lematz_words.append(lematz_token)
    return " ".join(lematz_words)
